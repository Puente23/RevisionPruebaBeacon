...
export 'BeaconsTools.dart';

class PuenteBeacons {
  PuenteBeacons._();
  static final PuenteBeacons _instance = PuenteBeacons._();
  late final _beaconScanner;
  Future<List<Beacon>> scan(FlutterBlue flutterBlue) async {
    try {
      await _beaconScanner.initializeScanner();
      final beacons = await _beaconScanner.getBeacons();
      return beacons;
    } catch (e) {
      if (kDebugMode) {
        print(e);
      }
      throw Exception('An error occurred while scanning for beacons');
    }
  }
